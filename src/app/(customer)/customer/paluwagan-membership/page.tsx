...
export default function CustomerPaluwaganMembershipPage() {
  ...
  const handleIdFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validTypes.includes(selected.type.toLowerCase())) {
        setIdFileError("Invalid file type. Please upload a JPG, JPEG, PNG, or PDF file.");
        setIdFile(null);
        setIdFileName("");
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!idFile && !idFileName) {
      setIdFileError("Government-issued ID is required to submit your Paluwagan application.");
      return;
    }
            ...
            <Card className="bg-white border border-slate-200/60 rounded-3xl shadow-xl max-w-3xl mx-auto flex flex-col max-h-[72vh] sm:max-h-[75vh] overflow-hidden">
              ...
              >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase flex items-center justify-between">
                        <span>Upload ID (Government-issued ID file) *</span>
                        <span className="text-[9px] text-emerald-600 font-semibold">(JPG, PNG, PDF)</span>
                      </label>
                      <div className="relative">
                        ...
                        <label
                          htmlFor="idFileInput"
                          className={`w-full flex items-center justify-between px-3 py-2 border rounded-xl text-xs font-medium cursor-pointer transition-colors ${
                            idFile || idFileName
                        ...
                        >
                          <span className="truncate max-w-[200px]">
                            {idFile ? idFile.name : idFileName ? idFileName : "Choose Government ID file..."}
                          </span>
                          <span className="flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-lg shrink-0">
                      ...
                      {idFileError && (
                        <p className="text-[10px] font-bold text-rose-500 mt-1 flex items-center gap-1">
                          <AlertCircle className="w-3 h-3" /> {idFileError}
                        </p>
                      )}
                    </div>
                    <div className="space-y-1">
                ...
                <div className="pt-2 text-right">
                  <Button
                    type="submit"
                    disabled={(!idFile && !idFileName) || isGeneratingPdf || !certifyCorrect || !agreeTerms}
                    variant="primary"
                    className="font-bold cursor-pointer"
                  >
                    {isGeneratingPdf ? "Generating Application PDF..." : "Submit Application"}
                  </Button>
                </div>
              </form>
              </div>
